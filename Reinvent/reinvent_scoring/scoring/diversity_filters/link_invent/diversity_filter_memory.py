...
class DiversityFilterMemory:
    ...
    def __init__(self):
        ...
        df_dict = {"Step": [], "Scaffold": [], "SMILES": [], "linker": [], "input":[]}
        self._memory_dataframe = pd.DataFrame(df_dict)

    def update(self, indx: int, score: float, smile: str, scaffold: str, linker: str, input: str, components: List, step: int):
        ...
        if not self.smiles_exists(smile):
            self._add_to_memory_dataframe(step, smile, scaffold, linker, input, component_scores)

    def _add_to_memory_dataframe(self, step: int, smile: str, scaffold: str, linker: str, input: str, component_scores: {} = None):
        data = []
        headers = []
