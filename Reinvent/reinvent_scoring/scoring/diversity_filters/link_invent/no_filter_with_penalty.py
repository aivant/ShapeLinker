...
from reinvent_scoring.scoring.diversity_filters.link_invent.base_diversity_filter import BaseDiversityFilter
    ...
    def update_score(self, score_summary, sampled_sequences: List, step=0) -> np.array:
        ...
        for i in score_summary.valid_idxs:
            if scores[i] >= self.parameters.minscore:
                ...
                self._add_to_memory(i, scores[i], smiles[i], decorations, sampled_sequences[i].output, sampled_sequences[i].input, score_summary.scaffold_log, step)
        return scores
