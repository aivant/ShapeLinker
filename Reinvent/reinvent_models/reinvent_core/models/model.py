...
class Model:
    ...
    def sample_smiles(self, num=128, batch_size=128, sampling_type = 'multinomial', k = None, p = 1.0, temperature = 1.0) -> Tuple[List, np.array]:
        """
        ...
        for size in batch_sizes:
            if not size:
                break
            seqs, likelihoods = self._sample(batch_size=size, sampling_type=sampling_type, k = k, p = p, temperature=temperature)
    ...
    def sample_sequences_and_smiles(self, batch_size=128, sampling_type = 'multinomial') -> Tuple[torch.Tensor, List, torch.Tensor]:
        seqs, likelihoods = self._sample(batch_size=batch_size, sampling_type = sampling_type)
        smiles = [self.tokenizer.untokenize(self.vocabulary.decode(seq)) for seq in seqs.cpu().numpy()]
        return seqs, smiles, likelihoods

    # @torch.no_grad()
    def _sample(self, batch_size=128, sampling_type = 'multinomial', k = None, p = 1.0, temperature = 1.0) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if sampling_type == 'multinomial':
            sequences, nlls = self._multinomial_sampling(sequences, nlls, hidden_state, input_vector, temperature)
        elif sampling_type in ['top_k', 'top_p', 'top_k_top_p']:
            sequences, nlls = self._top_sampling(sequences, nlls, hidden_state, input_vector, k, p)

        sequences = torch.cat(sequences, 1)
        return sequences.data, nlls
    
    def _multinomial_sampling(self, sequences, nlls, hidden_state, input_vector, temperature = 1.0):
        for _ in range(self.max_sequence_length - 1):
            ...
            if input_vector.sum() == 0:
                break
        return sequences, nlls

    def _top_sampling(self, sequences, nlls, hidden_state, input_vector, k = None, p = 1.0):
        if k is None:
        ...
        for _ in range(self.max_sequence_length - 1):
            ...
            rand_indices = torch.randint(0, k, (top_k_probs.shape[0], 1))
            input_vector = torch.gather(top_k_indices,1,rand_indices).view(-1)
            sequences.append(input_vector.view(-1, 1))
            nlls += self._nll_loss(log_probs, input_vector)
            if input_vector.sum() == 0:
                break
