...
def soft_distances(x, y, batch_x, batch_y, smoothness=0.01, atomtypes=None):
    ...
    if atomtypes is not None:
        ...
        atomic_radii = torch.cuda.FloatTensor(
            [170, 110, 152, 155, 180, 190, 150, 110], device=x.device
        )
    ...
    result = o3d.pipelines.registration.registration_ransac_based_on_feature_matching(
        ...
        3, [
            ...
            o3d.pipelines.registration.CorrespondenceCheckerBasedOnDistance(
                distance_threshold)
        ], o3d.pipelines.registration.RANSACConvergenceCriteria(5000000, 0.99999))
    return result


def ransac_registration(points1, points2, voxel_size: float = .6, return_score: bool = True):
    ...
    if return_score:
        ...
        return res.fitness * (np.asarray(res.correspondence_set).shape[0] / ((points1.shape[0] + points2.shape[0])/2))
...
def draw_registration_result(source, target, transformation):
    ...
    o3d.utility.Vector3dVector(source))
    target = o3d.geometry.PointCloud(
    o3d.utility.Vector3dVector(target))
...
class ElasticDistortion:
    ...
    def __init__(
        self, apply_distorsion: bool = True, granularity: ty.List = [0.2, 0.8], magnitude=[0.4, 1.6],
    ):
    ...
    @staticmethod
    def elastic_distortion(coords, atom_coords, granularity, magnitude):
        ...
        atom_coords = atom_coords + interp(atom_coords) * magnitude
        return coords, atom_coords

    @staticmethod
    def iterative_distortion(coords, atom_coords, granularity, magnitudes):
            blurx = np.ones((3, 1, 1, 1)).astype("float32") / 3
            blury = np.ones((1, 3, 1, 1)).astype("float32") / 3
            blurz = np.ones((1, 1, 3, 1)).astype("float32") / 3
            coords_min = coords.min(0)
            ...
            interp = scipy.interpolate.RegularGridInterpolator(ax, noise, bounds_error=0, fill_value=0)
            return [coords + interp(coords) * magnitude for magnitude in magnitudes], [atom_coords + interp(atom_coords) * magnitude for magnitude in magnitudes]

    def __call__(self, coords, atom_coords):
        # coords = data.pos / self._spatial_resolution
        if self._apply_distorsion:
            if random.random() < 0.95:
                for i in range(len(self._granularity)):
                    coords, atom_coords = ElasticDistortion.elastic_distortion(coords, atom_coords, self._granularity[i], self._magnitude[i],)
        return coords.astype(np.float32), atom_coords.astype(np.float32)

    def iterative(self, coords, atom_coords, num_magnitudes: int):
        if self._apply_distorsion:
            for i in range(len(self._granularity)):
                coords_mod, atom_coords_mod = ElasticDistortion.iterative_distortion(coords, atom_coords, self._granularity[i], np.linspace(1, self._magnitude[i], num_magnitudes))
        return [x.astype(np.float32) for x in coords_mod], [x.astype(np.float32) for x in atom_coords_mod]
