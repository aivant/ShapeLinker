...
class Molecules:
    ...
    @classmethod
    def from_list_of_smiles(cls, multi_smiles, number_of_conformers, optimize=True, device="cuda", torsion_noise=0):
        batches = list()
        atom_types = list()
        coords = list()
        b_num = 0
...
def emit_molecules_for_contrastive_training(list_of_smiles, number_of_conformers, device="cuda"):
    ...
    similar2 = Molecules.from_list_of_smiles(list_of_smiles, number_of_conformers, optimize=False, device=device)
    distant = Molecules.from_list_of_smiles(random.sample(list_of_smiles, len(list_of_smiles)), number_of_conformers, optimize=False, device=device)
...
def emit_molecules_for_contrastive_training_with_torsion_noise(list_of_smiles, number_of_conformers, device="cuda", degree_noise=10, mult=2):
    similar1 = Molecules.from_list_of_smiles(list_of_smiles, number_of_conformers, optimize=True, device=device)
    similar2 = Molecules.from_list_of_smiles(list_of_smiles, number_of_conformers, optimize=True, device=device, torsion_noise=degree_noise)
    distant = Molecules.from_list_of_smiles(list_of_smiles, number_of_conformers, optimize=True, device=device, torsion_noise=degree_noise*mult)
    return similar1, similar2, distant
...
class dMaSIF(nn.Module):
    ...
    def features(self, molecules: Molecules, i=1):
        ...
        # if (
        #     not self.use_mesh and "xyz" not in P
        ...
        #     P["xyz"],
        #     triangles=P["triangles"] if self.use_mesh else None,
        #     normals=None if self.use_mesh else P["normals"],
