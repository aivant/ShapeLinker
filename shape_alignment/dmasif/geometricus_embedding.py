...
class GeometricusPerc(torch.nn.Module):
    def __init__(self, geom_dim, chem_dim, latent_dim, latent_scale):
        super(GeometricusPerc, self).__init__()
        input_dim = geom_dim + chem_dim
        ...
        self.atomnet = AtomNet_MP(chem_dim)
        # self.relu = torch.nn.ReLU()
        # self.bn = torch.nn.BatchNorm1d(latent_dim)
        
    def forward(self, molecules: Molecules):
        # geometricus as embedding
        features = molecules.get_features(self.atomnet)
        sequences = batches_to_variable_length(features, molecules.surface_batches)
